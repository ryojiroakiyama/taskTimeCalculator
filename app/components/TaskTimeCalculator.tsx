...
const TaskTimeCalculator: React.FC = () => {
  ...
  const handleChange = (index: number) => {
    setAdditionalElements((prevState) =>
      prevState.map((item, i) => (i === index ? { ...item, active: !item.active } : item)),
    )
  }

  const totalAdditionalTime = additionalElements.reduce(
    (total, item) => total + (item.active ? item.value : 0),
    0,
  )
